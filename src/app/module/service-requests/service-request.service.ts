...
} from "./service-request.interface.js";
    ...
    return prisma.$transaction(
      async (tx) => {
        ...
        if (user.role !== "ADMIN") {
          throw new AppError(
            httpStatus.FORBIDDEN,
            "Only an admin can change the service request status",
          );
        }
        ...
        if (
          ...
          throw new AppError(
            httpStatus.BAD_REQUEST,
            `Cannot change service request status from ${currentStatus} to ${nextStatus}`,
          );
        }
