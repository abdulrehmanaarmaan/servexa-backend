...
import type {
  ...
  IScheduleWorkOrderPayload,
  IUpdateWorkOrderPayload,
  IUpdateWorkOrderStatusPayload,
  IWorkOrderQuery,
} from "./work-order.interface.js";
...
const createWorkOrder = async (
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (serviceRequest.workOrder) {
      throw new AppError(
        httpStatus.CONFLICT,
        "Work order already exists for this service request",
      );
    }
    ...
    if (serviceRequest.status !== "APPROVED") {
      throw new AppError(
        ...
        "Only approved service requests can be converted into work orders",
      );
    }
    ...
    if (serviceRequest.service.basePrice === null) {
      throw new AppError(
        httpStatus.BAD_REQUEST,
        "The selected service does not have a base price",
      );
    }
    ...
    await tx.jobStatusHistory.create({
      data: {
        ...
        reason:
          "Work order created from approved service request",
      },
    });
    ...
    await tx.auditLog.create({
      data: {
        ...
        newValue: {
          ...
          servicePrice:
            workOrder.servicePrice?.toString(),
...
const getMyTechnicianWorkOrders = async (
  user: IRequestUser,
  query: IWorkOrderQuery,
) => {
  const technician =
  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      if (
        ...
        throw new AppError(
          httpStatus.FORBIDDEN,
          "Only an admin can cancel a work order",
        );
      }
...
const scheduleWorkOrder = async (
  workOrderId: string,
  payload: IScheduleWorkOrderPayload,
  user: IRequestUser,
) => {
  return prisma.$transaction(
    async (tx) => {
      ...
      if (
        ...
        throw new AppError(
          httpStatus.BAD_REQUEST,
          "Completed work orders cannot be rescheduled",
        );
      }
      ...
      if (
        ...
        throw new AppError(
          httpStatus.BAD_REQUEST,
          "Cancelled work orders cannot be scheduled",
        );
      }
